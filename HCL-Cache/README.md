
The HCL Cache integrates with DynaCache and improves performance and scalability with features such as remote caching, real-time monitoring, and auto-tuning capabilities. To help you get started with the HCL Cache and [Redis](https://redis.io/), see the [implementation](Implementation.md) guide, and review the topics below.

- [Architecture](Architecture.md)
  - [Local and Remote caching](LocalAndRemoteCaching.md)
    - [Local cache auto-tuning](LocalCacheAutoTuning.md)
  - [Invalidation Support](Invalidations.md)
  - [Circuit Breakers](CircuitBreakers.md)
  - [Remote Cache Maintenance Processes](HCLCacheMaintenance.md)
  - [Remote Cache Tuning Configurations](RemoteCacheTuningConfigurations.md)
  
- [Monitoring](Monitoring.md) 
  - [WebSphere Cache Monitor](WebSphereCacheMonitor.md)
  - [Metrics Logging](logMetricsFrequency.md)
  - [Configurable Prometheus Metrics](MetricsConfiguration.md)

- [Redis Servers](RedisServer.md)
  - [Memory Management in Redis](RedisMemoryManagement.md)
  - [Bitnami Redis Installation](BitnamiRedisInstall.md)  
  - [HCL Cache in Redis](HCLCacheInRedis.md)  
  - [Use of Redis Replicas](RedisReplicas.md)

- [Configuration](Configuration.md)
  - [Redis Client Configuration](RedisClientConfig.md)
  - [Cache Configurations](CacheConfiguration.md)
  - [Development Environment](DevelopmentEnvironment.md)
  
- [Custom Caching](CustomCaching.md)

- [HCL Cache Manager](CacheManager.md)
  - [Utilities](Utilities.md)

- [Troubleshooting](Troubleshooting.md)

- [Change Log](ChangeLog.md)
